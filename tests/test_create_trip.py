...
# @pytest.xfail(reason = 'Negative test to verify that trip creation fails when required fields are missing')
@pytest.mark.asyncio
@pytest.mark.smoke()
async def test_create_trip_with_registration_without_destination(login):
    ...
    try:
        await expect(creation_page.launch_trip_button).to_not_be_visible(timeout=5000)
    except:
        logging.error("Trip created without destination, which should not happen.")